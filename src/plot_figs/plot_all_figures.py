...
# Get project root (assuming `model` and `utils` are siblings of `src`)
...
class PlotAllfigures:
    """Legacy monolithic plotting class.

    Note: New code should use modules in `src/plot_figs/`. This class is kept
    for backwards compatibility and as the implementation source that the new
    modules delegate to.
    """
    def __init__(self):
        pass
    def fig1_study_areas(self):
        ...
        def add_tmean():
            ...
            # Add histogram inset at bottom-right of ax3
        ...
        def add_prec():
            ...
            # Add histogram inset at bottom-right of ax4
        ...
        def add_ete0():
            ...
            # Add histogram inset at bottom-right of ax5
        ...
        def add_gdd10():
            ...
            # Add histogram inset at bottom-right of ax6
    ...
    def fig2_calibration(self):
        ...
        # Display names and subplot titles
        ...
        # Color settings: calibration=light blue, validation=light orange
        ...
        # Add legend to the bottom-right subplot (third row, second column)
        ax[5].legend(
    ...
    def fig3_irrigation_threshoulds(self):
        ...
        def plot_raster_layer(ax, file_path, data_type):
            ...
            # Add histogram inset at the bottom-right of the axis
        ...
        def irrigation_threshoulds():
            ...
            for data_type, file_path, position in subplot_configs:
                ...
                bbox = ax.get_position()   # Get subplot bbox [x0, y0, width, height]
                # Colorbar position: right above the subplot
                if position in [1,2]:
                    up = 0.11
                else:
                    up = 0.04
                cax_position = [
                    bbox.x0,        # align left
                    bbox.y1 + up,   # slightly above top
                    bbox.width + 0.01,     # same width as subplot
                    0.02           # height
                ]
                cbar = create_colorbar(fig, cax_position, config)
                # Set colorbar title above
            ...
            plt.tight_layout()
            # Slightly increase vertical spacing to avoid crowding
    ...
    def fig_baseline(self):
        ...
        def baseline_yield_irrigation(label):
            ...
            axes_dict = {}  # Save each subplot axis
            ...
            for data_type, file_path, position, scenario in subplot_configs:
                ax = create_geo_axis(fig, position)
                axes_dict[position] = ax  # Save axis
            ...
            # Probability density plots (ax5, ax6)
    ...
    def fig4_deficit_return(self):
        ...
        def create_colorbar(fig, ax_position, cmap, cbar_config, vmin, vmax, step):
            ...
            # Labels and ticks on top of the colorbar
        ...
        def plot_raster_layer(ax, file_path, data_type):
            ...
            ax.imshow(masked_data, origin='upper',transform=ccrs.PlateCarree(),
                        ...
                        norm=Normalize(vmin=vmin, vmax=vmax))
            # Hide ticks and remove geo frame
            ax.set_xticks([])
            ax.set_yticks([])
            ax.spines['geo'].set_visible(False)

            # Add a histogram inset at the bottom-right of the axis
            hist_masked_data = np.ma.masked_outside(data, vmin, vmax)
            # hist_masked_data = masked_data
        ...
        def plot_grid():
            ...
            for row_idx, period in enumerate(all_periods):
                for col_idx, (data_type, metric_filename) in enumerate(metrics):
                    ...
                    # Save data for KDE plots
                    masked_data = np.ma.masked_less_equal(raster_data, 0)
                    all_raster_data[data_type][period] = masked_data.compressed()  # Keep positive values
                    plot_geodata(ax)

                    # Add colorbar to the first row only
                    if row_idx == 0:
                ...
                # Add period label on the left of each row
    ...
    def fig5_future_results(self):
        ...
        # Replace labels and sort categories
    ...
    def fig6_plot_standardized_coefficients(self):
        ...
        # Store last axis used to show x-axis labels
        last_ax = None
        ...
        for i, (period, label) in enumerate(zip(periods, right_labels)):
            ...
            # Colorbar position is only for the first plot
            if i == 0:
            ...
            if i < len(periods) - 1:
                ...
                last_ax = ax  # Last axis shows x-axis ticks
    ...
    def fig_s3_plot_correlation_analysis(self):
        ...
        # Plot joint distribution of features and targets
...
def main():
    ...
    # Legacy optional runners (prefer using src/plot_figures.py):
